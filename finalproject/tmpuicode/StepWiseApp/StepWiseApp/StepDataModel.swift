...
class StepDataModel {
    ...
    func startTrackingSteps(onUpdate: @escaping (Int, Double, Double) -> Void, onError: @escaping (Error) -> Void) {
        ...
        self.pedometer.startUpdates(from: Date()) { [weak self] pedometerData, error in
            ...
            if let steps = pedometerData?.numberOfSteps {
                ...
                let goalCount = self.fetchCurrentGoalCount()
                    if stepInt >= goalCount && goalCount > 0 {
                        self.triggerGoalNotification()
                    }
    ...
    func fetchCurrentGoalCount() -> Int {
        guard let appDelegate = UIApplication.shared.delegate as? AppDelegate else {
        ...
        do {
            ...
            return Int(results.last?.goalCount ?? 0) // Assuming latest entry
        } catch {
            print("Failed to fetch goal count: \(error)")
            return 0
        }
    }
